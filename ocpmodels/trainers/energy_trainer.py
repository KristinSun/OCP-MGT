    ...
    def predict(
        ...
        for _, batch in tqdm(
            ...
            if per_image:
                ...
                return predictions
            # 存energy
            out_energy["energy"].extend(batch[0].y_relaxed.cpu().detach().numpy())
            out_energy["id"].extend(
                [str(i) for i in batch[0].sid.tolist()]
            )


        mae_enengy = np.mean(np.abs(np.array(out_energy["energy"]) - predictions["energy"]))
        print(mae_enengy)
