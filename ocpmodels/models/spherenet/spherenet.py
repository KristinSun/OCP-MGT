...
class SphereNet(BaseModel):
    ...
    def forward(self, batch_data):
        z, pos, batch = batch_data.atomic_numbers.long(), batch_data.pos, batch_data.batch
        if self.energy_and_force:
            pos.requires_grad_()
        edge_index = radius_graph(pos, r=self.cutoff, batch=batch)
        num_nodes = z.size(0)
        #if num_nodes != batch.size(0):
            #return torch.tensor([-1.525913953781128]).to(z.device)
        ...
        v = self.init_v(e, i)
        if v.size(0) != batch.size(0):
            return batch_data.y_relaxed.requires_grad_()
